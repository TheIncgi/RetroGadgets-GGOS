...
function utils.subAlias( a, aliases )
  local out = {}
  for i,j in ipairs(a) do
    out[j] = true
  end
  for id,ali in pairs(aliases) do
    if out[id] then
      out[id] = nil
      out[ali] = true
    end
  end
  return utils.keys(out)
end

--present in math as math.map
-- function utils.mathMap(x,a,b,c,d)
--   return (x-a)/(b-a) * (d-c) + c
-- end

function utils.startsWith( a, b, caseSense )
...
--vs
--utils.checkType({"number","string","table"}, x)
--TODO remove because typeMatches also does this, but with class support
--function utils.checkType( types, value )
 -- if type(types)==table then 
 --   for i,ty in pairs(types) do
 --     if type(value)==ty then return true end
 --   end
 --   return false
 -- else
...
function utils.matchesOption( name, option )
  ...
  local inWords = utils.splitWords(name, true)
  local opWords = utils.splitWords(option, true)
...
function utils.typeMatches( value, options )
  ...
  for _, opt in ipairs(options) do
    ...
    if opt:sub(1,6) == "class:" then
      ...
      local ok, cl = pcall( require,  cName..".lua" )
      if ok and isClass( value ) 
            and isClass( cl ) then
...
function utils.tryCatch( ... )
  ...
  local r2
  local r = {xpcall( args.try, function(...) r2 = {args.catch(...)} end )}
  local ret = {}
  if r[1] then
...
local lastYield = gdt.CPU0.Time
function utils.autoYield()
	local now = gdt.CPU0.Time
	if now - lastYield >= 1 / 50 then
		--yield()
		--print"Yield!"
		lastYield = gdt.CPU0.Time
	end
end
...
function utils.cprint( clr:number, ...)
	setFgColor( clr )
	print( ... )
	setFgColor(utils.ansiColor.white)
end
return utils
